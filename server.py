        ...
        if os.path.exists(comfy_settings_path):
            with open(comfy_settings_path, 'r', encoding='utf-8') as f:
                ...
                _settings_cache = {
                    ...
                    'huggingface_mirror': comfy_settings.get('WorkflowModelsDownloader.HuggingFaceMirror', ''),
...
def get_huggingface_base_url():
    """Get base URL for HuggingFace downloads (supports mirror)"""
    settings = load_settings()
    base = settings.get('huggingface_mirror', '').strip()
    if base:
        return base.rstrip('/')
...
def _download_model_thread(download_id, hf_repo, hf_path, filename, target_dir):
    ...
    try:
        ...
        # Determine base URL (supports mirror)
        base_url = get_huggingface_base_url()

        # Get file size first
        try:
            url = f"{base_url}/{hf_repo}/resolve/main/{hf_path}"
...
async def get_settings(request):
    ...
    try:
        ...
        masked = {
            ...
            'huggingface_mirror': settings.get('huggingface_mirror', ''),
...
async def update_settings(request):
    ...
    try:
        ...
        if 'huggingface_mirror' in data:
            # Mirror can be empty string to disable
            mirror = (data.get('huggingface_mirror') or '').strip()
            current['huggingface_mirror'] = mirror
